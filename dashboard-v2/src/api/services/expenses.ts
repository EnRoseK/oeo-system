...
const getExpenses = async ({ page, pageSize, limit, filters }: ServiceQuery) => {
...
const createExpense = async (data: any) => {
  return await axiosInstance.post(CREATE_EXPENSE, data).then((res) => res.data);
};

const updateExpense = async (id: number, data: any) => {
  return await axiosInstance.put(UPDATE_EXPENSE(id), data).then((res) => res.data);
};

const deleteExpense = async (id: number) => {
  return await axiosInstance.delete(DELETE_EXPENSE(id)).then((res) => res.status);
};
